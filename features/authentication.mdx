...
<Steps>
    ...
    <Step title="Create new Transactional email">
        Create a new transcactional email in Loops [here](https://app.loops.ai/transactional).
    </Step>
    ...
    <Step title="Set the transactional id">
      ...
      Copy it and set it in `hooks.server.ts` in the `EmailProvder` provider in the body of the `fetch` request.
```ts hooks.server.ts
